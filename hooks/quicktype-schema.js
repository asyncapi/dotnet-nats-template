...
const { quicktype, InputData, JSONSchemaInput, JSONSchemaStore, NewtonsoftCSharpTargetLanguage } = require("quicktype-core");

/**
 * Generate a c# structure from a schema. 
 * 
...
async function genSchema(schemaPath, schemaName, jsonSchema){
	...
	const { lines } = await quicktype({ lang: lang, inputData, rendererOptions: {
		namespace: filters.pascalCase(schemaName)+"NameSpace"
	}});
	await fs.promises.mkdir(schemaPath, { recursive: true }).catch(console.error);
	fs.mkdirSync(schemaPath, { recursive: true });
	fs.writeFileSync(Path.join(schemaPath, filters.pascalCase(schemaName)+".cs"), lines.join("\n"));
}
	...
	'generate:after': async (generator) => {
		...
		for (let [messageId, message] of allMessages) {
			...
			if(payloadSchema.type()+"" != "null"){
				const filepath = Path.join(generator.targetDir, "Dotnet.Nats.Client/messages/");
